...
def main():
    ...
    hyperparams = model_hyperparam(learning_rate=0.1,batch_size=128,epochs=1000,momentum=0.9,decay_factor=0.95,num_channels=3)
    
    directory = '/om2/user/shenwang/deeplearning/CIFAR/cifar-10-python/cifar-10-batches-py'
    model_folder = 'small_inception'
    resume_folder = 'corrupt_labels_40_12-06-2024_22-52-53'
    start_weights = 3850
    weights_file = 'model_weights'+str(start_weights)+'.pth'
    data_prefix = 'data'
    test_prefix = 'test'
    ...
    plotdir = os.path.join(directory, model_folder, resume_folder)
    ...
    checkpoint = torch.load(os.path.join(plotdir,weights_file), weights_only=True)
    ...
    for t in range(start_weights,5000,1):
        print(f"Epoch {t+1}\n-------------------------------")
